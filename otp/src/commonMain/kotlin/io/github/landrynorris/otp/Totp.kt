...
/**
 * Implementation of RFC 6238 Time-Based One-Time Password. Uses current time step as the challenge.
 * The time step is calculated as the Unix timestamp divided by [timeStep]. The timestamp can be
 * overridden using [setTime].
 */
...
data class Totp(
    ...
    /**
     * Gets the current time step as a challenge. Time step is calculated as t/[timeStep], where t
    ...
    /** Override the timestamp. This will replace the Unix timestamp in future pin calculations. */
    fun setTime(newTimeSeconds: Long) {
        overwrittenTime = newTimeSeconds
    }

    /**
     * Value in [0, 1)f describing the current fraction of the way through the current time step.
     */
    val progress: Float
        get() {

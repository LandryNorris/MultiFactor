...
suspend fun assertOccursWithin(
    duration: Long,
    message: String = "",
    predicate: suspend () -> Boolean,
) {
    val result =
        withTimeoutOrNull(duration.milliseconds) { while (!predicate()) delay(1.milliseconds) }
    assertNotNull(
        result,
        "Predicate${if(message.isNotEmpty()) " " else ""}" +
            "$message did not return true within $duration ms",
    )
}

fun initKoin(
    otpRepository: OtpRepository = createOtpRepository(),
    ...
    startKoin { modules(repositoryModule(otpRepository, passwordRepository, settingsRepository)) }
}

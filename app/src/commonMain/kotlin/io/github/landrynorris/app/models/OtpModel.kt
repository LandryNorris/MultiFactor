...
fun OtpModel.toEntry() =
    when (otp) {
        is Hotp -> OtpEntry(id, otp.secretBase32, otp.name, otp.counter, 0L)
        is Totp -> OtpEntry(id, otp.secretBase32, otp.name, null, 1L)
    }

fun OtpEntry.toModel() =
    OtpModel(id = id, otp = if (type == 0L) Hotp(secret, name, count ?: 0) else Totp(secret, name))

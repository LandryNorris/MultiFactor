...
class OtpComponent(private val context: ComponentContext, private val repository: OtpRepository) :
    ComponentContext by context, OtpLogic {
    override val state = MutableStateFlow(OtpScreenState())
    override val createOtpLogic = CreateOtpComponent(childContext("create")) { addOtp(it) }

    init {
        CoroutineScope(Dispatchers.Default).launch {
            repository.getOtpModelFlow().collect { models ->
                state.update { it.copy(otpList = models.map { model -> model.toState() }) }
            }
        }
    }

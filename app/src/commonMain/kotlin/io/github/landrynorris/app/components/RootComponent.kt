...
class RootComponent(context: ComponentContext, private val crypto: Crypto) :
    ComponentContext by context, Root, KoinComponent {
    ...
    @Serializable
    sealed class Config {
        @Serializable object OtpConfig : Config()

        @Serializable object PasswordConfig : Config()

        @Serializable object Settings : Config()

        @Serializable object About : Config()
    }
}
